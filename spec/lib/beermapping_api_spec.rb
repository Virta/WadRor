...
describe "BeermappingApi" do
  ...
  it 'when http get does not return entries, a valid table is returned' do
    canned_answer = <<-EOF
      <?xml version='1.0' encoding='utf-8' ?><bmp_locations><location><id></id><name></name><status></status><reviewlink></reviewlink><proxylink></proxylink><blogmap></blogmap><street></street><city></city><state></state><zip></zip><country></country><phone></phone><overall></overall><imagecount></imagecount></location></bmp_locations>
    EOF
    stub_request(:get, /.*jokunen/).to_return(body: canned_answer, headers: { 'Content-Type' => "text/xml" })
    places = BeermappingApi.places_in('jokunen')

    expect(places.empty?).to eq(true)

  end

  it 'when http get returns several entries, a table in matching size is returned' do
    canned_answer = <<-EOF
